  ...
  return (
    ...
    >
      <div className="relative aspect-square w-full overflow-hidden">
        <Link href={trackPath(track)} className="absolute inset-0 z-0">
          <Image
            src={track.cover}
          ...
          />
          <span className="sr-only">{track.title} の詳細ページへ</span>
        </Link>
        <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-night-deep/80 via-night-deep/10 to-transparent" />
        ...
        {isActivePlaying && (
          <div
            className="absolute bottom-4 left-4 z-10 flex items-end gap-0.5"
            aria-hidden="true"
          >
            {[0, 1, 2].map((i) => (
              <span
        ...
        <div>
          <h3 className="font-serif text-xl font-medium leading-snug text-ink-light">
            <Link
              ...
              className="transition-colors hover:text-aurora-ice focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-aurora-mint rounded"
            >
              {track.title}
            </Link>
          </h3>
          {track.titleEn ? (
            <p className="mt-0.5 font-jp text-xs text-ink-soft/50">{track.titleEn}</p>
          ) : null}
